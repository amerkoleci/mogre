...
generic<typename T> void ObjectTable::Add(intptr_t pointer, T object, Mogre::IDisposable^ owner)
{
	//if (pointer == nullptr)
...
void ObjectTable::AddObjectOwner(Mogre::IDisposable^ object, Mogre::IDisposable^ owner)
{
	if (object == nullptr)
...
generic<typename T> void ObjectTable::AddOwnerTypeLookup(Object^ owner, T object)
{
	if (object == nullptr)
		throw gcnew ArgumentNullException("object");
...
bool ObjectTable::Remove(intptr_t pointer)
{
	...
	if (IsInstanceOf<Mogre::IDisposable^>(object))
	{
		IDisposable^ disposableObject = dynamic_cast<Mogre::IDisposable^>(object);
...
Dictionary<Mogre::IDisposable^, Mogre::IDisposable^>^ ObjectTable::Ownership::get()
{
	return _ownership;
}
