...
void ObjectTable::DisposeOfObjectAndDependents(Mogre::IDisposable^ disposable)
{
	if (disposable == nullptr || disposable->IsDisposed || !_ownership->ContainsKey(disposable))
		return;
	...
	for each(Mogre::IDisposable^ dependent in dependents)
	{
		delete dependent;
	}
...
array<Mogre::IDisposable^>^ ObjectTable::GetDependents(Mogre::IDisposable^ disposable)
{
	auto allDependents = gcnew List<Mogre::IDisposable^>();
...
void ObjectTable::GetDependents(Mogre::IDisposable^ disposable, List<Mogre::IDisposable^>^ allDependents)
{
	// TODO: Make a reverse dictionary of _ownership to avoid this O(N)
	for each(KeyValuePair<Mogre::IDisposable^, Mogre::IDisposable^> d in _ownership)
	{
		// Get the object (child) of the disposable arg
		Mogre::IDisposable^ dependent = d.Key;
